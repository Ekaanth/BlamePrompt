...
pub fn run(output: &str) {
        ...
        for (i, line) in lines[start..end].iter().enumerate() {
            ...
            for vuln in VULN_PATTERNS {
                ...
                if !vuln.file_extensions.is_empty()
                    && !vuln.file_extensions.iter().any(|ext| *ext == file_ext)
                {
                    continue;
                }
    ...
    if !findings.is_empty() {
        ...
        cwe_sorted.sort_by(|a, b| b.1.1.cmp(&a.1.1));
    ...
    md.push_str("---\n\n");
    md.push_str("*Generated by [BlamePrompt](https://github.com/anthropics/blameprompt) — AI Code Vulnerability Scanner*\n");
