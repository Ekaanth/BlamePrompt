...
pub fn run(output: &str) {
    ...
    for r in &all_receipts {
        models_seen.entry(r.model.clone()).or_default().push(relative_path(&r.file_path));
    }
    ...
    for (model_id, files) in &models_seen {
        ...
        if classification.license == ModelLicense::OpenSource {
            ...
            warnings.push(format!(
                ...
                model_id,
                unique_files.len(),
                unique_files.iter().take(5).cloned().collect::<Vec<_>>().join(", ")
            ));
        }
    }
    ...
    md.push_str(&format!("| Open-source model warnings | {} |\n\n", warnings.len()));
    ...
    for (model_id, files) in &models_seen {
        ...
        unique.dedup();
        md.push_str(&format!("| {} | {} | {} | {:?} | {} |\n",
            c.display_name, c.vendor, license_str, c.deployment, unique.len()));
    }
    md.push_str("\n");
    ...
    if flagged_count > 0 {
        ...
        md.push_str("2. **Add attribution** — Some licenses (Llama, Apache 2.0) require attribution.\n");
        md.push_str("3. **Check MAU thresholds** — Llama-family models restrict commercial use above 700M MAU.\n");
    }
    md.push_str("4. **Document model usage** — Keep this scan as part of your compliance evidence.\n");
    md.push_str("5. **Review periodically** — Model licenses can change between versions.\n\n");

    md.push_str("---\n\n*Generated by [BlamePrompt](https://github.com/anthropics/blameprompt) License Compliance Scanner*\n");
