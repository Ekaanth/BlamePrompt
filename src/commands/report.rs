...
pub fn generate_report(
    ...
    write_executive_summary(&mut md, &repo_name, &entries, &all_receipts, total_commits, from, to);
    ...
    // Footer
    writeln!(md, "\n---").ok();
    writeln!(md, "*Generated by [BlamePrompt](https://github.com/blameprompt/blameprompt) v0.1.0*").ok();
...
fn write_executive_summary(
    ...
    let total_lines: u32 = receipts.iter().map(|r| {
    ...
    writeln!(md, "> Generated: {} | Period: {} | Commits: {}\n", today, period, total_commits).ok();
    writeln!(md, "## Executive Summary\n").ok();
    writeln!(md, "| Metric | Value |").ok();
    writeln!(md, "|--------|-------|").ok();
    writeln!(md, "| Total commits analyzed | {} |", total_commits).ok();
    writeln!(md, "| Commits with AI-generated code | {} ({:.1}%) |", commits_with_ai, pct).ok();
    writeln!(md, "| Total AI-assisted sessions | {} |", session_ids.len()).ok();
    writeln!(md, "| Total files modified by AI | {} |", unique_files.len()).ok();
    ...
    let uncommitted_count = entries.iter()
...
fn write_ai_vs_human(md: &mut String, receipts: &[&Receipt]) {
    ...
    for r in receipts {
        let lines = if r.line_range.1 >= r.line_range.0 { r.line_range.1 - r.line_range.0 + 1 } else { 0 };
        let entry = by_file.entry(relative_path(&r.file_path)).or_insert((0, r.model.clone()));
        entry.0 += lines;
    }
...
fn write_cost_analysis(md: &mut String, receipts: &[&Receipt], entries: &[audit::AuditEntry]) {
    ...
    if session_count > 0 {
        writeln!(md, "| Avg cost per session | ${:.3} |", total_cost / session_count as f64).ok();
    }
    if !entries.is_empty() {
        writeln!(md, "| Avg cost per AI commit | ${:.3} |", total_cost / entries.len() as f64).ok();
    }
    writeln!(md).ok();
    ...
    for (model, (count, cost)) in &models {
        ...
        writeln!(md, "| {} | {} | ${:.2} | {:.1}% |", display, count, cost, pct).ok();
    }
    writeln!(md).ok();
}
...
fn write_user_contributions(md: &mut String, receipts: &[&Receipt]) {
    ...
    users.sort_by(|a, b| b.1.2.partial_cmp(&a.1.2).unwrap_or(std::cmp::Ordering::Equal));
...
fn write_time_analysis(md: &mut String, receipts: &[&Receipt]) {
    ...
    for r in receipts {
        if let Some(rt) = r.ai_response_time_secs {
            model_response_times.entry(r.model.clone()).or_default().push(rt);
        }
    }
...
fn write_security_audit(md: &mut String, findings: &[SecurityFinding], receipts: &[&Receipt]) {
    ...
    writeln!(md, "- **Prompts with secrets detected**: {} ({:.1}%)",
             prompts_with_secrets,
             if total_prompts > 0 { prompts_with_secrets as f64 / total_prompts as f64 * 100.0 } else { 0.0 }
    ).ok();
...
fn write_model_comparison(
    ...
    writeln!(md, "|---------|----------|-----------|").ok();
    writeln!(md, "| Closed-source | {} | ${:.2} |", closed_sessions, closed_cost).ok();
    writeln!(md, "| Open-source | {} | ${:.2} |", open_sessions, open_cost).ok();
    writeln!(md).ok();
    ...
    for (i, r) in receipts.iter().enumerate() {
        ...
        let entry = by_vendor.entry(c.vendor.clone()).or_insert((license_str.to_string(), 0, 0.0));
        entry.1 += 1;
        entry.2 += r.cost_usd;
    }
    ...
    vendors.sort_by(|a, b| b.1.2.partial_cmp(&a.1.2).unwrap_or(std::cmp::Ordering::Equal));
    for (vendor, (license, sessions, cost)) in &vendors {
        writeln!(md, "| {} | {} | {} | ${:.2} |", vendor, license, sessions, cost).ok();
    }
    writeln!(md).ok();
}
...
fn write_file_heatmap(md: &mut String, receipts: &[&Receipt]) {
    ...
    for r in receipts {
        let lines = if r.line_range.1 >= r.line_range.0 { r.line_range.1 - r.line_range.0 + 1 } else { 0 };
        let entry = by_file.entry(relative_path(&r.file_path)).or_insert((0, 0, String::new(), String::new()));
        entry.0 += 1; // edits
        entry.1 += lines;
    ...
    writeln!(md, "| Rank | File | AI Edits | AI Lines | Last AI Edit | Primary Model |").ok();
    writeln!(md, "|------|------|----------|----------|-------------|---------------|").ok();
    let mut files: Vec<_> = by_file.into_iter().collect();
    files.sort_by(|a, b| b.1.1.cmp(&a.1.1));
    for (rank, (file, (edits, lines, last_edit, model))) in files.iter().enumerate().take(20) {
        let display = model_classifier::display_name(model);
        writeln!(md, "| {} | {} | {} | {} | {} | {} |", rank + 1, file, edits, lines, last_edit, display).ok();
    }
    writeln!(md).ok();
}
...
fn write_session_analysis(md: &mut String, receipts: &[&Receipt]) {
    ...
    for r in receipts {
        let entry = session_stats.entry(r.session_id.clone())
            .or_insert((0, HashSet::new(), 0.0, String::new()));
    ...
    sessions.sort_by(|a, b| b.1.0.cmp(&a.1.0));

    writeln!(md, "### Top Sessions by Message Count").ok();
    writeln!(md, "| Session ID | Messages | Files | Est. Cost | Prompt Summary |").ok();
    writeln!(md, "|-----------|----------|-------|-----------|---------------|").ok();
    for (sid, (msgs, files, cost, prompt)) in sessions.iter().take(10) {
        let short_sid: String = sid.chars().take(8).collect();
        writeln!(md, "| {} | {} | {} | ${:.2} | \"{}\" |", short_sid, msgs, files.len(), cost, prompt).ok();
    }
    writeln!(md).ok();
}
...
fn write_prompt_details(md: &mut String, entries: &[audit::AuditEntry]) {
    ...
    for entry in entries {
        ...
        let sha_display = if entry.commit_sha == "uncommitted" {
            ...
            format!("Commit `{}`", &entry.commit_sha[..std::cmp::min(8, entry.commit_sha.len())])
        };
        ...
        for r in &entry.receipts {
            writeln!(md, "#### {} | `{}` via {}\n", relative_path(&r.file_path), r.model, r.provider).ok();
...
fn write_recommendations(
    ...
    // Check Opus usage
    let opus_count = receipts.iter().filter(|r| r.model.to_lowercase().contains("opus")).count();
    let total = receipts.len();
    if total > 0 && (opus_count as f64 / total as f64) > 0.3 {
        writeln!(md, "{}. **Cost Optimization**: {:.0}% of sessions used Opus models (higher cost).",
                 rec_num, opus_count as f64 / total as f64 * 100.0
        ).ok();
        writeln!(md, "   Consider using Sonnet for simpler tasks to reduce costs.\n").ok();
        rec_num += 1;
    }

    // Security
    if !findings.is_empty() {
        writeln!(md, "{}. **Security**: {} session(s) contained secrets in prompts.",
                 rec_num, findings.len()
        ).ok();
    ...
    let open_count = classifications.iter()
        .filter(|(_, c)| c.license == model_classifier::ModelLicense::OpenSource)
        .count();
    if total > 0 && (open_count as f64 / total as f64) < 0.1 {
        writeln!(md, "{}. **Open-Source Opportunity**: {:.1}% of sessions use open-source models.",
                 rec_num, open_count as f64 / total as f64 * 100.0
        ).ok();
    ...
    if rec_num == 1 {
        writeln!(md, "No specific recommendations at this time. Usage patterns look good!").ok();
    }
}
